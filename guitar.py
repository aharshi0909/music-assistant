...
def extract_guitar_only_segment(path, min_db=-35):
...
def extract_guitar_features(path):
    try:
        y, sr = extract_guitar_only_segment(path)
...
def compare_guitar_tracks(file1, file2):
    f1 = extract_guitar_features(file1)
    f2 = extract_guitar_features(file2)
    if f1 is None or f2 is None:
        return {"error": "Invalid or silent guitar audio"}
...
result = compare_guitar_tracks("aud1.wav", "aud2.wav")
print(result)
