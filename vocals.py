...
def extract_vocal_segment(path, min_db=-35):
...
def extract_vocal_features(path):
    try:
        y, sr = extract_vocal_segment(path)
        ...
        if chroma_cens.shape[1] == 0:
            return None
        beat_chroma = librosa.util.sync(chroma_cens, beats, aggregate=np.median)
...
def compare_vocal_tracks(file1, file2):
    f1 = extract_vocal_features(file1)
    f2 = extract_vocal_features(file2)
    if f1 is None or f2 is None:
        return {"error": "Invalid or silent vocal audio"}
...
result = compare_vocal_tracks("vocal1.wav", "vocal2.wav")
print(result)
