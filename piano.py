...
def extract_piano_segment(path, min_db=-35):
...
def extract_piano_features(path):
    try:
        y, sr = extract_piano_segment(path)
...
def compare_piano_tracks(file1, file2):
    f1 = extract_piano_features(file1)
    f2 = extract_piano_features(file2)
    if f1 is None or f2 is None:
        return {"error": "Invalid or silent piano audio"}
...
result = compare_piano_tracks("piano1.wav", "piano2.wav")
print(result)
